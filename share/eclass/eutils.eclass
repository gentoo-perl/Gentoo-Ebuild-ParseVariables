...
eshopts_push() {
	...
	if [[ $1 == -[su] ]] ; then
		...
		__ESHOPTS_SAVE__[$i]=$-
		[[ $# -eq 0 ]] && return 0
		set "${@/c}" || die "eshopts_push: bad options to set: $*"
	fi
}
...
eshopts_pop() {
	...
	if [[ ${s} == "shopt -"* ]] ; then
		...
		set +${-/c}   || die "eshopts_pop: sanity: invalid shell settings: $-"
		set -${s/c}   || die "eshopts_pop: sanity: unable to restore saved shell settings: ${s}"
	fi
}
